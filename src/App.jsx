...
export default function App() {
  ...
  const printInvoiceItems = useCallback(async (invoice, itemsToPrint, detailed = false, includeInvoiceCopy = false) => {
    ...
    if (includeInvoiceCopy) {
      ...
      let origFileHtml = '';
      if (inv.file_path) { try { const url = await db.getInvoiceFileUrl(inv.file_path); if (url) { origFileHtml = inv.file_type?.includes('pdf') ? `<div class="orig-file"><p class="ofl">Original Invoice</p><iframe src="${url}" style="width:100%;height:250mm;border:none;"></iframe></div>` : `<div class="orig-file"><p class="ofl">Original Invoice</p><img src="${url}" style="width:100%;max-height:260mm;object-fit:contain;"/></div>`; } } catch(e){} }
  ...
  const generateReturnPDF = useCallback(async (detailed = true, includeInvoiceCopy = false) => {
    ...
    if (includeInvoiceCopy) {
      ...
      for (const invId of invIds) {
        ...
        let origFileHtml = '';
        if (inv.file_path) { try { const url = await db.getInvoiceFileUrl(inv.file_path); if (url) { origFileHtml = inv.file_type?.includes('pdf') ? `<div class="page pb"><div class="orig-file"><p class="ofl">Original Invoice Document</p><iframe src="${url}" style="width:100%;height:260mm;border:none;"></iframe></div></div>` : `<div class="page pb"><div class="orig-file"><p class="ofl">Original Invoice Document</p><img src="${url}" style="width:100%;max-height:265mm;object-fit:contain;"/></div></div>`; } } catch(e){} }
